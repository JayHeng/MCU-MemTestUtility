...
class memTesterUi(QMainWindow, memTesterWin.Ui_memTesterWin):
    ...
    def _getVal32FromMemRegText( self, text, ident ):
        val32 = uidef.kInvalidMemRegVal
    ...
    def findReadbackRegsValue( self ):
        ...
        memRegsCfgDict['regsVal'][0] = self._getVal32FromMemRegText(text, 'Status Register: ')
        text = self.textEdit_displayWinReg.toPlainText()
        memRegsCfgDict['regsVal'][1] = self._getVal32FromMemRegText(text, 'Custom Register1: ')
        text = self.textEdit_displayWinReg.toPlainText()
        memRegsCfgDict['regsVal'][2] = self._getVal32FromMemRegText(text, 'Custom Register2: ')
    ...
    def setMemVendor( self ):
        try:
            ...
            self.toolCommDict['memVendor'] = self.comboBox_memVendor.currentIndex()
    ...
    def setMemType( self ):
        try:
            ...
            self.toolCommDict['memType'] = self.comboBox_memType.currentIndex()
            #print(self.memType)
    ...
    def _getMemChipInfo( self ):
        ...
        self.toolCommDict['memChip'] = self.comboBox_memChip.currentIndex()
    ...
    def _initMemSpeed( self ):
        memSpeed = str(self.toolCommDict['memSpeed']) + 'MHz'
        for i in range(self.comboBox_memSpeed.count()):
            if self.comboBox_memSpeed.itemText(i) == memSpeed:
                self.comboBox_memSpeed.setCurrentIndex(i)
                break
